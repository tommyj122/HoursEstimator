...
class KitInstance:
    ...
    def display_name(self) -> str:
        """Return the kit code plus the user-facing name.

        Default names include a compact instance number. Custom names are used
        verbatim and are never modified.
        """
        name = self.custom_name or f"{self.definition.default_name} #{self.index}"
        return f"{self.definition.code} {name}"
...
class Element:
    ...
    def compact_indices(self) -> None:
        """Re-number kit instances so they are sequential per kit code."""
        counts: Dict[str, int] = {}
        for kit in self.kits:
            code = kit.definition.code
            counts[code] = counts.get(code, 0) + 1
            kit.index = counts[code]
