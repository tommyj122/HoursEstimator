...
class Estimate:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for el_code, el in self.elements.items():
            kits = []
            for kit in el.kits:
                kits.append(
                    {
                        "definition": {"code": kit.definition.code},
                        "index": kit.index,
                        "custom_name": kit.custom_name,
                        "param_values": kit.param_values,
                        "deliverables": kit.deliverables,
                    }
                )
            data["elements"][el_code] = {"kits": kits, "notes": el.notes}
        return data
    ...
    def from_json(cls, data: str) -> "Estimate":
        ...
        for el_code, el_data in raw.get("elements", {}).items():
            ...
            element.notes = el_data.get("notes", "")
